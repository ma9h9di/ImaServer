    ...
    check: function (data, user) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!data.hasOwnProperty('token')) {
                return reject(new err(pv.errCode.token_field_not_found, undefined, {params: ['token']}).jsonErr());
            } else {
                if (!data.token === user.session[0].token) {
                    return reject(new err(pv.errCode.authentication.token_illegal_sing_up).jsonErr());
                }
            }

            if (!data.hasOwnProperty('first_name')) {
                return reject(new err(pv.errCode.arguments_not_found, undefined, {params: ['fist_name']}).jsonErr());
            } else {
                if (data.first_name === '') {
                    return reject(new err(pv.errCode.invalid_arguments, 'first name can`t empty', {params: ['fist_name']}).jsonErr());
                }
            }
            if (!data.hasOwnProperty('last_name')) {
                ...
                if (data.last_name === '') {
                    return reject(new err(pv.errCode.invalid_arguments, 'last name can`t empty', {params: ['last_name']}).jsonErr());
                }
            }
            if (!data.hasOwnProperty('email')) {
                ...
                if (data.email !== '' && data.email.indexOf('@') <= 0) {
                    return reject(new err(pv.errCode.invalid_arguments, 'this mail not valid', {params: ['email']}).jsonErr());
                }
            }
