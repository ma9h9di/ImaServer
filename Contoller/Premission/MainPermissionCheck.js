    ...
    check: function (input, client) {
        ...
        if (pv.permission.notNeedTokenApi.hasOwnProperty(input.method)) {
            ...
            if (data.phone_number.length < 11)
                return new err(pv.errCode.authentication.phone_number_not_valid, 'phone number less than 11').jsonErr();
            if (data.phone_number[0] !== '+')
                return new err(pv.errCode.authentication.phone_number_not_valid, 'phone number format not valid').jsonErr();
